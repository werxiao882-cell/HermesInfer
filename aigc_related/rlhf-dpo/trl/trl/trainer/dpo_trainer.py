...
class DataCollatorForPreference(DataCollatorMixin):
    ...
    def torch_call(self, examples: list[list[int] | Any | dict[str, Any]]) -> dict[str, Any]:
        ...
        # 对于 Decoder-only 模型，Prompt 必须使用左侧填充（Left Padding）。
        # 这样可以确保在拼接 Prompt 和 Completion 时，Prompt 的最后一个 Token 紧邻 Completion 的第一个 Token，
        # 从而维持因果语言模型（Causal LM）的位置连续性和推理一致性。
...
class DPOTrainer(BaseTrainer):
    ...
    def concatenated_inputs(
        ...
        功能：
        1. 将 Batch 中的 `chosen` 和 `rejected` 回答在第 0 维（Batch 维度）进行拼接。
        2. 对不同长度的回答序列进行对齐（Padding），确保能够组成统一形状的张量。
        3. 对多模态数据（如图像特征 `pixel_values`）进行相应的复制与拼接。

        `batch` 字典结构说明：
        ------------------------------------------
        - prompt_input_ids:      [batch_size, prompt_len] 提示词。
        - chosen_input_ids:      [batch_size, chosen_len] 偏好回答。
        - rejected_input_ids:    [batch_size, rejected_len] 非偏好回答。
        - padding_value:         用于填充的 Token ID。
        ------------------------------------------

        返回说明:
            返回一个包含拼接后张量的字典，Batch 维度扩展为 `2 * batch_size`。
        """
        output = {}
    ...
    def dpo_loss(
        ...
        r"""
        计算模型与参考模型之间的 DPO 损失与奖励指标。

        功能：
        该函数根据 DPO 算法核心公式，计算当前策略模型相对于参考模型的偏好损失。参数与公式对应关系如下：
        - chosen_logps:      对应公式中的 log π_\theta(y_w|x)，即策略模型对“选中回答”的对数概率。
        - rejected_logps:    对应公式中的 log π_\theta(y_l|x)，即策略模型对“拒绝回答”的对数概率。
        - ref_chosen_logps:  对应公式中的 log π_{ref}(y_w|x)，即参考模型对“选中回答”的对数概率。
        - ref_rejected_logps:对应公式中的 log π_{ref}(y_l|x)，即参考模型对“拒绝回答”的对数概率。
        - loss_type:         损失函数类型。默认 "sigmoid" 即标准 DPO（log σ(…)），也支持 "ipo"、"hinge" 等变种。
        返回说明：
            - losses:          当前批次的损失值张量。
            - chosen_rewards:   计算出的选中回答的奖励值（基于对数比值）。
            - rejected_rewards: 计算出的拒绝回答的奖励值（基于对数比值）。
        """
        device = self.accelerator.device

        # Get the log ratios for the chosen and rejected responses
        # NOTE：计算的是DPO Loss公式中的 log π(y_w|x) - log π_ref(y_w|x)
        chosen_logratios = chosen_logps.to(device) - (not self.reference_free) * ref_chosen_logps.to(device)
        # NOTE: 计算的是DPO Loss公式中的 log π(y_l|x) - log π_ref(y_l|x)
        ...
        if self.f_divergence_type == FDivergenceType.ALPHA_DIVERGENCE:
            ...
            # NOTE: 计算的是DPO Loss公式中的  π_\theta(y_w|x) - π_\theta(y_l|x)
            logratios = chosen_logps - rejected_logps
            if self.reference_free:
                ref_logratios = torch.tensor([0], dtype=logratios.dtype, device=logratios.device)
            else:
                # NOTE：计算的是DPO Loss公式中的 log π_{ref}(y_w|x) - log π_{ref}(y_l|x)
                ref_logratios = ref_chosen_logps - ref_rejected_logps

            logratios = logratios.to(self.accelerator.device)
            ref_logratios = ref_logratios.to(self.accelerator.device)
            # NOTE： 计算的是DPO Loss公式中的 log π(y_w|x) -  log π_ref(y_w|x) - ( log π(y_l|x) -  log π_ref(y_l|x))
        ...
        # beta 是 DPO 损失的一个温度参数，通常取值范围在 0.1 到 0.5 之间。
        # 当 beta 趋近于 0 时，参考模型的影响将被忽略。label_smoothing 参数体现了我们对标签的不确定性，
        # 用于计算一个更为保守的 DPO 损失。
    ...
    def concatenated_forward(
        ...
        功能：
        1. 调用 `concatenated_inputs` 准备 2N 大小的 Batch 数据。
        2. 处理 Decoder-only 与 Encoder-Decoder 架构的输入拼接逻辑。
            - 如果是 Decoder-only 架构的话，直接拼接提示词和回答。
            - 如果是 Encoder-Decoder 架构的话，分别处理编码器和解码器的输入。
        3. 执行模型 Forward 得到 logits，表示每个位置的 Token 预测分布。
        4. 结合 labels 和 loss_mask，利用 `selective_log_softmax` 提取目标 Token 的对数概率，并通过累加（Sum）得到公式中的序列级对数概率logπ(y|x)，最终提取出回答部分的 `chosen_logps` 和 `rejected_logps`。

        参数说明：
        ------------------------------------------
        - model:      执行前向传播的模型（策略模型或参考模型）。
        - batch:      原始 DataLoader 输出的 Batch。
        - is_ref_model: 是否为参考模型调用。
        ------------------------------------------

        返回说明:
            包含 chosen/rejected 的 logps、logits 均值以及可能的辅助 loss。
        """
        num_examples = batch["prompt_input_ids"].shape[0]
        # NOTE：拼接数据，Batch大小变为2N, 过一次模型，节省计算时间
        concatenated_batch = self.concatenated_inputs(batch, padding_value=self.pad_token_id)
        ...
        # NOTE: prompt_input_ids指的是拼接后的问题，completion_input_ids指的是拼接后的答案，都是2N大小
        ...
        if self.is_encoder_decoder:
            ...
            if self.max_length is not None and self.max_length < attention_mask.size(1):
                ...
                if "token_type_ids" in concatenated_batch:
                    ...
                    # NOTE：张量中每一行的有效Token（非 Padding Token）全部向左对齐，移除序列开头的填充。
                    attention_mask, input_ids, loss_mask = flush_left(attention_mask, input_ids, loss_mask)
            ...
            """
            在Decoder-only模型中, DPO的实现是将问题和答案在序列维度dim=1拼接后, 送入到模型的前向传播中。
            虽然是一起输入的,但DPO并不关心问题部分的预测结果,只计算答案部分的loss。
            """
            outputs = model(input_ids, **model_kwargs)
            
            # NOTE: 对于序列中的每一个位置，logits包含了模型对词表中每一个Token预测概率的原始线性得分。
            logits = outputs.logits
    ...
    def get_batch_loss_metrics(
        ...
        r"""
        计算单个 Batch 的 DPO 损失与各项监控指标。

        功能：
        1. 调度前向传播：根据配置选择选择用 Liger Kernel 优化路径或标准的 `concatenated_forward` 路径。
        2. 获取参考概率：从 `batch` 中读取预计算的参考对数概率，若无则实时调用参考模型进行计算。
        3. 多损失计算：支持多种 DPO 变种（Sigmoid, IPO, Hinge 等），并处理权重衰减。
        4. 评估指标统计：计算 `chosen` 和 `rejected` 的奖励值、奖励准确率、对数概率以及对数输出。
        5. 指标聚合：利用 Accelerator 在多显卡间同步指标数据，确保日志统计的全局准确性。

        `batch` 字典结构说明：
        ------------------------------------------
        - prompt_input_ids:      [batch_size, seq_len] 提示词 Token ID。
        - chosen_input_ids:      [batch_size, seq_len] 偏好回答的 Token ID。
        - rejected_input_ids:    [batch_size, seq_len] 非偏好回答的 Token ID。
        - pixel_values:          (可选, VLM) 图像特征。
        - ref_chosen_logps:      (可选) 参考模型对 chosen 序列预计算的对数概率。
        - ref_rejected_logps:    (可选) 参考模型对 rejected 序列预计算的对数概率。
        ------------------------------------------

        返回 (Returns):
            tuple(losses, metrics):
            - losses: 该 Batch 的平均损失标量 (torch.Tensor)，用于执行 `loss.backward()`。
            - metrics: 字典格式。包含 'rewards/chosen', 'rewards/accuracies', 'logps/chosen' 等监控指标。
        """
        metrics = {}
        ...
        if self.args.use_liger_kernel:
            ...
            # NOTE: 计算的是公式中的log π_theta(y_w|x)和log π_theta(y_l|x)
            ...
            if "ref_chosen_logps" in batch and "ref_rejected_logps" in batch:
                ...
                # NOTE：计算参考模型的log π_ref(y_w|x)和log π_ref(y_l|x)
            ...
            for idx, loss_type in enumerate(self.loss_type):
                # NOTE：计算完所需的log π_theta(y_w|x)、log π_theta(y_l|x) 、log π_ref(y_w|x)、log π_ref(y_l|x)后，计算DPO损失
    ...
    def compute_loss(
        ...
        功能：
        1. 接收来自 DataLoader 的 Batch 数据（包含 prompt 和成对的 chosen/rejected 回答）。
        2. 调用 `get_batch_loss_metrics` 进行模型前向传播并计算 DPO 损失与指标。
        3. 存储指标并返回 Loss 以进反向传播。

        `inputs` 字典结构说明 (在 VLM/LLM 下有所不同)：
        ------------------------------------------
        - prompt_input_ids:      [batch_size, seq_len] 用户问题的 Token ID。
        - prompt_attention_mask: [batch_size, seq_len] 问题掩码。
        
        - chosen_input_ids:      [batch_size, seq_len] 偏好回答chosen的 Token ID。
        - chosen_attention_mask: [batch_size, seq_len] 偏好回答掩码。
        
        - rejected_input_ids:    [batch_size, seq_len] 非偏好回答rejected的 Token ID。
        - rejected_attention_mask:[batch_size, seq_len] 非偏好回答掩码。
        
        - pixel_values:          (可选, 仅 VLM) [batch_size, ...] 图像特征张量。
        - image_sizes:           (可选, 仅 VLM) 原始图像尺寸。
        
        - ref_chosen_logps:      (可选) 预计算得到的参考模型对 chosen 的对数概率。
        - ref_rejected_logps:    (可选) 预计算得到的参考模型对 rejected 的对数概率。
        ------------------------------------------
        """
