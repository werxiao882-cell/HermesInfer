...
class GRPOTrainer(BaseTrainer):
    ...
    def _get_per_token_logps_and_entropies(
        ...
        """
        计算序列中每个 Token 的对数概率（Log-probs）和信息熵（Entropies）。

        该方法主要用于训练过程中的损失计算，通过前向传播获取模型对补全（Completion）部分的预测分布。
        为了节省显存，它支持将大批次数据切分为较小的子批次（Micro-batches）进行逐一处理。

        关于 pixel_values 的作用：
        在多模态模型（VLM）中，`pixel_values` 承载了图像的像素级特征。为了把“视觉信息”转换成模型能听懂的“话”，然后和文本一起塞进模型。

        Args:
            model: 执行推理的模型（Policy 或 Reference 模型）。
            input_ids: 完整的 Token ID 序列（Prompt + Completion）。
            attention_mask: 注意力掩码。
            logits_to_keep: 需要保留的 Logits 长度（通常等于补全部分的长度）。
            batch_size: 子批次大小，用于控制显存峰值。
            compute_entropy: 是否计算并返回信息熵。
            pixel_values: 图像像素张量（针对 VLM）。
            image_grid_thw: 图像网格维度信息（针对 Qwen2-VL 等模型）。
            num_images: 每个样本包含的图像数量。
            ...其他多模态相关的 Mask 和 Size 参数。

        Returns:
            tuple: (logps, entropies)
                - logps: 每个补全 Token 的对数概率。
                - entropies: 每个位置的预测分布熵（若启用）。
        """
        ######################## 1. 批次切分与多模态数据准备：管理显存并将图像/文本对齐 ##########################
        ...
        for start in range(0, input_ids.size(0), batch_size):
            ...
            # 处理多模态图像分片逻辑
            if image_grid_thw is not None and pixel_values is not None:
                # 在某些模型（如 Qwen2-VL）中，pixel_values 是打平的一维列表，
                # 需要根据 image_grid_thw 指示的每个图像的 patch 数量进行动态切分。
            ...
            ######################## 2. 模型前向传播与 Logits 提取：获取原始预测分布 ##########################
            logits = model(**model_inputs).logits
            
            # 这里的 Shift 逻辑非常关键：
            # 模型在位置 t 的 logits 用于预测位置 t+1 的 token。
            # 排除最后一个值，因为它对应的是预测序列之外的“下一个”Token。
            logits = logits[:, :-1, :]  # (B, L-1, H)
            
            # 只保留补全（Completion）部分的 Logits，排除 Prompt 部分
            logits = logits[:, -logits_to_keep:, :]  # (B, logits_to_keep, H)
            
            # 温度缩放：调整分布的平滑度，RLHF 中常用此技巧稳定训练。
            logits = logits / self.temperature
            ######################## 2. 模型前向传播与 Logits 提取：获取原始预测分布 ##########################

            ######################## 3. 概率提取与信息熵计算：计算具体的 Log-probs ##########################
            # 提取目标 Token（即补全文本本身）在预测分布中的 Log 概率
            completion_ids = input_ids_batch[:, -logits_to_keep:]
            logps = selective_log_softmax(logits, completion_ids)  # compute logprobs
            all_logps.append(logps)

            # 如果需要计算熵（用于监控或某些 Loss），在此执行
            if compute_entropy:
                with torch.no_grad():
                    # 熵越大代表模型预测越不确定（分布越均匀）
    ...
    def _prepare_inputs(self, generation_batch: dict[str, torch.Tensor | Any]) -> dict[str, torch.Tensor | Any]:
        ...
        if mode == "train":
            ...
            if self._step % generate_every == 0 or self._buffered_inputs is None:
                # 1. 核心：生成文本、调用奖励函数评分，并计算组内优势 Advantage
                generation_batch = self._generate_and_score_completions(generation_batch)
                # 2. 多模态：针对 Qwen2-VL 等模型，将图像像素按网格切分，准备训练输入
                generation_batch = split_pixel_values_by_grid(generation_batch)
                # 3. 打乱：随机洗牌当前批次中的样本顺序，增加训练稳定性
                generation_batch = shuffle_sequence_dict(generation_batch)
                # 4. 切分：将大生成批次切分为多个微批次，用于后续梯度累积训练
    ...
    def _calculate_rewards(self, inputs, prompts, completions, completion_ids_list):
        """
        计算给定补全序列的奖励值，支持多种奖励函数（模型基础、同步函数或异步函数）。

        该方法执行以下操作：
        1. **初始化**：准备奖励存放张量，并解析输入数据中的附加列作为奖励函数的关键字参数。
        2. **同步奖励运行**：
            - 对于模型基础的奖励函数（`nn.Module`），执行推理并获取 Logits 作为分数。
            - 对于普通的 Python 同步函数，直接调用获取结果。
        3. **异步奖励运行**：将所有异步标明的奖励函数提交到事件循环中并行执行，以提高效率。
        4. **有效性检查**：检测是否有样本在所有奖励函数中均未获得有效分数（NaN），并输出调试信息。
        5. **结果汇总**：使用 `accelerate.gather` 汇总所有 GPU 进程计算出的奖励值，确保全局归一化。

        Args:
            inputs (`list[dict]`): 原始输入批次数据。
            prompts (`list[str]`): 解码后的 Prompt 文本列表。
            completions (`list[str]`): 解码后的 Completion 文本列表。
            completion_ids_list (`list[list[int]]`): Completion 的 Token ID 列表。

        Returns:
            `torch.Tensor`: 形状为 (num_samples, num_reward_funcs) 的奖励张量。
        """
        ######################## 1. 初始化与关键字参数准备 ##########################
    ...
    def _generate_and_score_completions(self, inputs: list[dict[str, torch.Tensor | Any]]) -> dict[str, torch.Tensor | Any]:
        """
        生成文本补全，调用奖励函数评分，并计算组内相对优势（Advantages）。

        该方法是 GRPO 训练流程的核心步骤，执行以下操作：
        1. **输入处理**：解析 prompt 和多模态数据（图像），并应用聊天模板。
        2. **文本生成**：使用模型生成补全内容（Completions），记录 token IDs 和生成时的 logprobs。
        3. **概率计算**：在无梯度模式下，计算当前策略模型和参考模型（Reference Model）对生成序列的 per-token logprobs，用于后续 KL 散度约束和重要性采样。
        4. **奖励评分**：运行所有定义的奖励函数（同步或异步），为每个生成结果打分。
        5. **优势估计（GRPO 核心）**：在同一 Prompt 的不同生成结果（Group）之间执行归一化（减去均值，除以标准差），将绝对 Reward 转化为相对 Advantage。
        6. **指标记录**：收集并记录奖励均值、标准差、补全长度等监控指标。

        Args:
            inputs (`list[dict[str, torch.Tensor | Any]]`): 包含 "prompt" 和可选多模态列（"images"/"image"）的数据列表。

        Returns:
            `dict[str, torch.Tensor | Any]`: 包含训练所需的所有张量，包括：
                - `prompt_ids` & `completion_ids`: 填充后的 token IDs。
                - `advantages`: 计算出的组内相对优势值。
                - `old_per_token_logps`: 策略模型在生成时的概率（用于重要性采样修正）。
                - `ref_per_token_logps`: 参考模型的概率（用于 KL 惩罚）。
                - 其他多模态特征张量（如像素值、图像网格信息等）。
        """
        device = self.accelerator.device
        mode = "train" if self.model.training else "eval"
        ...
        # 在RLHF中，prompt和completion的padding方式不同，prompt左侧padding，completion右侧padding，目的是为了避免prompt中的pad token影响attention计算
        ...
        ######################## 3. 概率计算：在无梯度模式下，计算当前策略模型和参考模型（Reference Model）对生成序列的 per-token logprobs ##########################
        ...
        # 机制：依赖一个显式的 Reward Model (RM)。这个 RM 通常是一个经过标兵数据训练的分类模型（Scalar Output），用来预测人类对回复的偏好。
        # 特点：需要额外的 Critic 网络来估计状态价值（Value Function），通过对比 RM 分数和 Critic 预测值来计算优势（Advantage）。
        # 2. DPO (Direct Preference Optimization)
        # 机制：完全不需要独立的 Reward Model。它通过数学变换，发现最优策略与奖励函数之间存在对数线性关系。
        # 理论：它直接利用 当前模型与 Reference 模型之间的 log-probability 差值（对数概率比）来隐含地表示奖励。其 Loss 函数直接在偏好数据（chosen/rejected）上优化。
        # 3. GRPO (Group Relative Policy Optimization)
        # 机制：同样需要 Reward Score，但它的创新在于获取评分的方式和基准（Baseline）的计算：
        # 评分方式：通常使用一组 Reward Functions（奖励函数模板）。这些函数可以是基于规则的（如：格式检查、是否包含正解、长度惩罚），也可以是调用外部模型（如：逻辑验证器）。
        # 核心优势：它不再需要 Critic 网络。它是通过对同一 Prompt 生成的一组（Group）回复进行奖励对比，通过组内归一化（减均值、除方差）来直接计算相对优势。
        rewards_per_func = self._calculate_rewards(inputs, prompts, completions, completion_ids_list)
        ######################## 4. 奖励评分：运行所有定义的奖励函数（同步或异步），为每个生成结果打分 ##########################

        ######################## 5. 优势估计（GRPO 核心）：在同一 Prompt 的不同生成结果（Group）之间进行归一化 ##########################
        # 根据第四步得到的 rewards_per_func，计算每个样本的优势值（Advantages），在GRPO实现中，优势函数的计算方法是：
        # 1. 按组计算均值和标准差：对于同一 Prompt 生成的多个回复（一个组），计算每个奖励函数的均值和标准差。
        # 2. 归一化奖励：对于每个生成的回复，减去对应组的均值，并除以标准差，得到归一化的奖励值。
        num_generations = self.num_generations if mode == "train" else self.num_generations_eval
    ...
    def get_off_policy_mask(
        ...
        """
        根据 DeepSeek-V3 论文中的离策略序列掩码（Off-Policy Sequence Mask）逻辑，判定是否保留采样序列。

        该掩码旨在解决强化学习中的“分布偏移”问题：当采样时的模型（Old Policy）与当前训练的模型（Current Policy）差异过大时，
        该样本的梯度可能会导致训练不稳定。

        判定规则：
        如果满足以下任一条件，则保留（Keep）该序列：
        1. 该序列具有正向优势（Advantage >= 0）。
        2. 该序列的平均 KL 散度在阈值范围内（KL <= delta）。

        Args:
            advantages (`torch.Tensor`): 形状为 (B, 1) 的优势值张量。
            per_token_logps (`torch.Tensor`): 当前策略模型对补全 Token 的 log-probabilities。
            sampling_per_token_logps (`torch.Tensor`): 采样（生成）时模型对补全 Token 的 log-probabilities。
            mask (`torch.Tensor`): 标识有效补全 Token 的掩码矩阵。
            off_policy_threshold (`float`): KL 散度的允许阈值（delta）。

        Returns:
            `torch.Tensor`: 形状为 (B, 1) 的布尔/数值掩码，1.0 表示保留，0.0 表示丢弃。
        """
        ######################## 1. 计算 Token 级别的 KL 散度 ##########################
        # forward KL div: log(pi_old) - log(pi_theta)
        # 这里的 pi_old 是采样时的概率，pi_theta 是当前模型的概率
        kl_div = sampling_per_token_logps - per_token_logps.detach()
        ######################## 1. 计算 Token 级别的 KL 散度 ##########################

        ######################## 2. 计算序列级别的平均 KL 散度 ##########################
        # 对整个序列（排除 Padding）计算平均 KL 散度，作为策略偏离程度的量化指标
        seq_kl_sum = (kl_div * mask).sum(dim=1, keepdim=True)
        avg_seq_kl = seq_kl_sum / mask.sum(dim=1, keepdim=True).clamp(min=1.0)
        ######################## 2. 计算序列级别的平均 KL 散度 ##########################

        ######################## 3. 执行过滤判定条件 ##########################
        # 对应论文逻辑：Keep if (Advantage >= 0) OR (Avg_KL <= delta)
        # 即使模型偏离较大，但如果这个回复比组内平均水平好（Adv > 0），我们也倾向于保留它进行学习
    ...
    def _compute_loss(self, model, inputs):
        """
        计算 GRPO 的训练损失，支持多种 Loss 类型（GRPO, SAPO, BNPO 等）。

        该方法执行以下操作：
        1. **概率采样**：计算当前策略模型对文本序列的 log-probabilities。
        2. **优势处理**：结合预计算的 Advantage 值，并根据 OPSM 逻辑应用离策略过滤。
        3. **权重修正**：计算重要性采样权重（Importance Sampling Ratio）修正分布偏移。
        4. **多样化 Loss**：实现 GRPO（裁剪后的目标函数）、SAPO 或 KL 约束等多种优化目标。
        5. **归一化**：根据 Token 数量或 Batch 大小对损失进行平衡。

        Args:
            model: 正在训练的模型（Policy Model）。
            inputs: 包含 Prompt, Completion, Advantages, Logprobs 等的输入字典。

        Returns:
            torch.Tensor: 计算后的标量损失值。
        """
        ######################## 1. 概率与熵计算：获取模型对补全文本的 Log-probs 和信息熵 ##########################
        prompt_ids, prompt_mask = inputs["prompt_ids"], inputs["prompt_mask"]
        completion_ids, completion_mask = inputs["completion_ids"], inputs["completion_mask"]
        input_ids = torch.cat([prompt_ids, completion_ids], dim=1)
        attention_mask = torch.cat([prompt_mask, completion_mask], dim=1)
        ...
        """
        该步骤提取模型对生成结果（Completion）中每个 token 的预测概率。
        
        逻辑示例：
          Prompt: "上海在哪里？"
          Completion: "上海在中国。"
          模型输入序列: [Prompt] + [上, 海, 在, 中, 国, 。]
        
        计算方式：对于自回归模型，我们要计算的是 P(Completion | Prompt)。
          - P("上") = 模型在输入 [Prompt] 后，预测下一个词为 "上" 的概率。
          - P("海") = 模型在输入 [Prompt + "上"] 后，预测下一个词为 "海" 的概率。
          - ... 依次类推。
        
        在代码中，`logits_to_keep` 会处理 Logits 偏移（Shift），使得结果 Tensor 的第 i 个位置
        正好对应 Completion 中第 i 个 token 的对数概率。同时也会计算信息熵（Entropy）用于后续多样性调优。
        """
        ...
        ######################## 3. 优势处理与离策略掩码 (OPSM)：判定是否需要丢弃漂移严重的序列 ##########################
        # Compute the loss
        advantages = inputs["advantages"]
        ...
        ######################## 4. 重要性采样权重与 KL 散度：修正模型漂移并应用 KL 惩罚 ##########################
        ...
        if self.beta != 0.0:
            ...
            # 计算 KL 散度的无偏估计 (或近似值)：KL = exp(log_ref - log_pi) - (log_ref - log_pi) - 1
            # 这是标准的 Schulman 近似公式，比简单的 log(pi) - log(ref) 更稳定。
            per_token_kl = (
                torch.exp(ref_per_token_logps - per_token_logps) - (ref_per_token_logps - per_token_logps) - 1
            )
            # Importance sampling correction for the KL divergence
            # 如果开启了偏差修正，KL 散度也会受到重要性采样权重 coef_1 的修正
            if self.args.use_bias_correction_kl:
                per_token_kl = per_token_kl * coef_1
        ######################## 4. 重要性采样权重与 KL 散度：修正模型漂移并应用 KL 惩罚 ##########################

        ######################## 5. 核心损失计算：实现 GRPO（裁剪）、SAPO 等具体目标函数逻辑 ##########################
        ...
        if self.loss_type == "cispo":
            ...
            #############################################################################################
            # --- PPO/GRPO 核心裁剪逻辑 ---
            # 1. 计算重要性采样比率的裁剪值 coef_2，将其限制在 [1-eps, 1+eps] 范围内
            ...
            # 2. 计算两个损失分支：
            # per_token_loss1: 原始重要性比率 * 优势值
            # per_token_loss2: 裁剪后的重要性比率 * 优势值
            per_token_loss1 = coef_1 * advantages
            per_token_loss2 = coef_2 * advantages
            
            # 3. 取两者的最小值：这是代理目标函数 (Surrogate Objective)
            # 它确保了模型不会为了获得极高的 Reward 而产生过大的参数更新。
            # 加负号是因为我们是在进行最小化优化 (Gradient Descent)。
            #############################################################################################
        ...
        if self.beta != 0.0:
            #############################################################################################
            # --- KL 惩罚项 ---
            # 最终的训练目标是 Maximize (Reward - beta * KL)
            # 转化为最小化 Loss 就是：Minimize (-Reward + beta * KL)
            # 这里的 per_token_loss 已经是 -Reward，所以加上 beta * per_token_kl
            per_token_loss = per_token_loss + self.beta * per_token_kl
            #############################################################################################
        ######################## 5. 核心损失计算：实现 GRPO（裁剪）、SAPO 等具体目标函数逻辑 ##########################

        ######################## 6. 损失归一化与指标记录：最终 Loss 汇总并上报训练状态 ##########################
        mode = "train" if self.model.training else "eval"
        ...
        # 额外的训练统计：计算 CLIP 比例（即有多少 Token 的梯度被裁剪了）
        # 这有助于监控训练稳定性，如果 clip_ratio 过高，说明策略偏移太快，需要减小学习率或增加采样步数。
