...
class PPOTrainer(BaseTrainer):
    ...
    def step(
        ...
        # NOTE：将query和response进行拼接, model_inputs["data"]["input_ids"].shape = [batch_size, seq_len]
        # model_inputs["data"]["attention_mask"].shape = [batch_size]
        ...
        with torch.no_grad():
            ####################################################################################################################
            # NOTE：将问题和答案拼接，放入模型的前向传播来计算π_old的logprobs和values
        ...
        with torch.no_grad():
            ...
            if full_kl_penalty:
                ...
                # NOTE：使用reward score和对数概率去计算reward，也就是公式里面的r_t
            ...
            t = time.time()
            """
            计算优势函数, 用于衡量某个动作相对于平均水平的好坏.
            优势函数的计算方式是A=Q-V, 其中Q是动作的预期回报, V是状态的价值估计.
            """
    ...
    def batched_forward_pass(
        ...
        for i in range(math.ceil(bs / fbs)):
            ...
            # NOTE：将问题和答案拼接放入模型的前向传播，获取logits和values。在PPO里面，value的获取方式是在模型的最后加一个线性层作为value head。
            ...
            # NOTE：计算 token 的对数概率。注意：logits 偏移一位以对齐下一位的 input_ids
            # 这行代码计算的是PPO公式中策略部分的核心组件：动作的对数概率 log π(a_t|s_t)
    ...
    def compute_advantages(
        ...
        """
        计算优势函数 (Advantages) 和回报目标值 (Returns)。
        该函数采用 GAE (Generalized Advantage Estimation) 算法来平衡方差和偏差。

        参数:
            values (torch.FloatTensor): 价值函数预测值，形状为 (batch_size, response_length)
            rewards (torch.FloatTensor): Token 级别的奖励信号 (包含 KL 惩罚和 RM 分数)，形状为 (batch_size, response_length)
            mask (torch.FloatTensor): 文本掩码，用于忽略 Padding 部分，形状为 (batch_size, response_length)

        返回:
            tuple: (values, advantages, returns)
        """
        lastgaelam = 0
        advantages_reversed = []
        gen_len = rewards.shape[-1]

        # 1. 预处理：应用掩码，确保只对 Response 部分进行计算
        values = values * mask
        ...
        # 2. 核心计算：自后向前遍历轨迹，递归计算 GAE
        for t in reversed(range(gen_len)):
            # 获取下一时刻的价值估计 V(s_{t+1})，如果是最后一个词则为 0
            nextvalues = values[:, t + 1] if t < gen_len - 1 else 0.0

            # 计算单步 TD 误差 (δ_t)
            # 公式: δ_t = r_t + γ * V(s_{t+1}) - V(s_t)
            # 这衡量了当前动作带来的即时奖励加上未来期望与当前预测价值的差值
            delta = rewards[:, t] + self.config.gamma * nextvalues - values[:, t]

            # 计算 GAE 值 (Â_t)
            # 公式: Â_t = δ_t + (γ * λ) * Â_{t+1}
            # 这里通过 λ 对未来误差进行指数衰减累加，以平滑优势信号
            lastgaelam = delta + self.config.gamma * self.config.lam * lastgaelam
            advantages_reversed.append(lastgaelam)

        # 3. 后处理：翻转结果并计算 Return
        # 将反向追加的结果翻转回正常的时间顺序
        advantages = torch.stack(advantages_reversed[::-1]).transpose(0, 1)

        # 计算 Return (R_t)，它作为价值网络 (Critic) 训练的目标值 (Target)
        # 公式: R_t = Â_t + V(s_t)
        returns = advantages + values

        # 优势白化：将优势函数归一化为均值为 0，方差为 1，以提高 PPO 策略更新的稳定性
