...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def get_rope_index(
        ...
        **kwargs,
        ...
        中文说明:
        基于图片和视频的尺寸计算 3D 旋转位置编码 (RoPE) 的索引。该工具函数预期输入为 `视觉 + 文本` 的序列，
        否则会报错。对于纯文本序列，请依赖模型自动推断的 position ids。在视觉和文本混合的序列中，
        视觉 token 使用 3D RoPE（时间、高度、宽度维度），而文本 token 使用标准的 1D RoPE。

        示例:
            时间 patch 数: 3; 高度 patch 数: 2; 宽度 patch 数: 2
            每个视觉输入会产生 (时间 x 高度 × 宽度) 个位置。在这里：3 x 2 × 2 = 12 个总位置。

            时间维度的位置 ID 间隔为:
                `interval = 每秒 token 数 * 时间 patch 大小 / 帧率(fps)`

                如果 fps = 1; tokens_per_second = 25; temporal_patch_size = 2，时间 ID 在每个时间 patch 会增加 50:
                `[0, 0, 0, 0, 50, 50, 50, 50, 100, 100, 100, 100]`

            高度 ID 按行重复: `[0, 0, 1, 1, ...]`
            宽度 ID 按列交替: `[0, 1, 0, 1, ...]`
            文本 token 遵循标准的 1D RoPE，位置 ID 相应地以步长为 `1` 增长。

        参数:
            input_ids (`torch.LongTensor` 形状为 `(batch_size, sequence_length)`):
                输入序列 token 在词表中的索引。如果提供了 padding，默认会被忽略。
            mm_token_type_ids (`torch.IntTensor` 形状为 `(batch_size, sequence_length)`):
                将输入序列中的每种模态匹配到不同值的 token 类型 ID，即：文本 (0)，图像 (1)，视频 (2)。
            image_grid_thw (`torch.LongTensor` 形状为 `(num_images, 3)`, *可选*):
                每张图片在 LLM 中特征形状的时间、高度和宽度维度大小。
            video_grid_thw (`torch.LongTensor` 形状为 `(num_videos, 3)`, *可选*):
                每个视频在 LLM 中特征形状的时间、高度和宽度维度大小。
            attention_mask (`torch.Tensor` 形状为 `(batch_size, sequence_length)`, *可选*):
                用于避免在 padding token 索引上执行注意力的掩码。掩码值选择在 `[0, 1]` 之间：
