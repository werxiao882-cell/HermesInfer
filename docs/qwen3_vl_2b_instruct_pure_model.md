# Qwen3-VL-2B-Instruct 纯模型结构解析

> 剥离推理框架，仅关注模型本身的计算结构与数据流。
> 所有 shape 基于 Qwen3-VL-2B-Instruct 实际配置。
> 与 Qwen3-0.6B 的关键差异：多模态输入、Vision Tower、MRoPE、DeepStack、自回归生成。
...
spatial_merge_size = 2             <-- 2x2 patch 合并
...
操作:
  pos_embed: nn.Embedding(2304, 1024)  <-- 48x48 学习位置网格
  对每张图: 从 48x48 双线性插值到实际 (h, w)
  h = h + pos
...
通俗解释:
  ...
  48x48 是学习到的"标准网格"，对不同尺寸的图像做插值适配。
```

### 3.3 VisionBlock — ViT 的基本单元 (x24)
...
### 3.5 PatchMerger — 2x2 patch 合并

```
作用: 把 2x2 相邻 patch 合并为 1 个 token, 减少序列长度

操作:
  _merge2x2:
    每张图 (t, h, w, 1024) -> 2x2 折叠 -> (t, h/2, w/2, 4096)
...
通俗解释:
  2x2 合并类似"降采样"：把 4 个相邻的小 patch 拼成 1 个大 patch。
...
### 4.3 Decoder Layer — 文本解码器的基本单元 (x28)
...
操作:
  ...
  3. MRoPE (3D 旋转位置编码, 详见 4.6):
     q, k = MRotaryEmbedding(pos3d, q, k)

  4. Flash Attention (因果, GQA):
...
## 五、LM Head — 从隐藏状态预测下一个 token

```
输入: hidden: (seq_len, 2048), cu_seqlens_q: (num_seqs+1,)

操作:
  1. Last-token gather (Prefill 时):
     last_indices = cu_seqlens_q[1:] - 1  <-- 每条序列的最后一个 token
     x = hidden[last_indices]
     -> (num_seqs, 2048)

     Decode 时: x 直接就是 (B, 2048)

  2. 线性投影:
     weight: (151936, 2048)  <-- 与 Embedding 共享 (tie_word_embeddings=True)
     logits = x @ weight^T
...
通俗解释:
  经过 28 层 decoder 后，最后一个 token 的隐藏状态包含了"到目前为止所有上下文"的信息。
  LM Head 要回答的问题是："基于这些信息，下一个最可能出现的词是什么？"

  它通过计算隐藏向量和词表中每个词向量的点积来衡量"匹配程度"。
  点积越大，说明隐藏向量的语义和那个词越接近。

  Weight Tying: LM Head 和 Embedding 共享同一组权重。
  语义上：编码和解码使用同一套语义空间。
  节省约 3.11 亿参数 (151936 x 2048)。

  Gumbel-max 采样: 通过加入 Gumbel 噪声实现带温度的随机采样。
  temperature 越高，采样越随机；越低，越接近贪心 (argmax)。
```
...
```
假设输入:
  文本: "What is in this image?"  -> token_ids: [系统+用户文本, 约 20 个]
  图像: 1 张 32x32 patch 的图 (t=1, h=32, w=32)
...
2. 文本 Embedding:
   token_ids: [系统文本, 151652, 151655x256, 151653, 用户文本]
   -> Embedding -> (seq_len, 2048)

3. Scatter visual_emb:
   x[image_token_spans] = visual_emb  <-- 覆盖 image_pad 位置
   -> (seq_len, 2048)

4. 28 层 Decoder:
   Layer 0: x[img_idx] += deepstack[0]  <-- 注入 ViT 层 5 特征
   Layer 1: x[img_idx] += deepstack[1]  <-- 注入 ViT 层 11 特征
   Layer 2: x[img_idx] += deepstack[2]  <-- 注入 ViT 层 17 特征
...
6. LM Head:
   取最后一个 token 的隐藏状态
   -> (2048,)
   -> logits = x @ weight^T -> (151936,)
   -> 采样 -> 第一个生成的 token

7. 同时: 所有 token 的 K/V 写入 KV cache
   -> 后续 Decode 阶段可以直接 attend 到图像特征

===== Decode 阶段 (自回归) =====

每次生成一个 token:

1. 输入: 上一个生成的 token ID
   -> Embedding -> (1, 2048)

2. 28 层 Decoder (不跑 Vision Tower):
   每层的 Attention 用 Paged Attention 从 KV cache 读取历史 K/V
   (包括 Prefill 时写入的图像 token 的 K/V)
   -> (1, 2048)

3. Final RMSNorm -> (1, 2048)

4. LM Head -> logits: (151936,)

5. 采样 -> 下一个 token

6. 新 token 的 K/V 写入 KV cache

重复直到生成 EOS (151645) 或达到 max_tokens
```
...
-----------------------------------------------------
Vision Tower:
  PatchEmbed3D                Conv3d: 3*2*16*16*1024 + 1024 = 1,573,888
...
| 特性 | Qwen3-0.6B | Qwen3-VL-2B-Instruct |
|------|-----------|----------------------|
| **任务** | 纯文本生成 | 多模态生成 (VQA/对话) |
| **输入模态** | 纯文本 | 文本 + 图像 + 视频 + 混合 |
...
| **推理模式** | Prefill + Decode | Prefill + Decode (相同) |
| **KV Cache** | 有 | 有 (含图像 token 的 K/V) |
| **LM Head** | 有 | 有 (相同) |
...
5. 图像 K/V 缓存复用
   +--------------------------------------------------+
   |  Prefill 时: 图像 token 的 K/V 写入 KV cache    |
   |  Decode 时: Paged Attention 直接读取             |
   |                                                  |
   |  无需重跑 Vision Tower                           |
   |  图像特征在整个生成过程中持续可用                |
   +--------------------------------------------------+
